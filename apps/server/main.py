                    ...
                    if node_name == "agent":
                        ...
                        if isinstance(node_output, dict):
                            ...
                            for msg in messages:
                                ...
                                # AI 응답 메시지
                                if msg_type == "ai" or (isinstance(msg, dict) and msg.get("type") == "ai"):
                                    content = getattr(msg, "content", None) or (msg.get("content") if isinstance(msg, dict) else None) or str(msg)
                        ...
                        if isinstance(node_output, dict):
                            ...
                            for msg in messages:
                                ...
                                if msg_content:
                                    print(f"      📦 Tool result: {str(msg_content)[:100]}...")
                                    try:
                                        # Tool 결과가 JSON 문자열일 수 있음
                                        tool_result = json.loads(msg_content) if isinstance(msg_content, str) else msg_content
                                        if isinstance(tool_result, dict):
                                            if tool_result.get("success") and tool_result.get("ui_tree"):
                                                print(f"      ✅ UI Tree found, sending...")
                                                event_dict = {
                                        ...
                                        print(f"      ⚠️ Failed to parse tool result: {e}")
                                        pass
            
            # 완료 이벤트
            yield ServerSentEvent(data=json.dumps({"type": StreamEventType.DONE.value}, ensure_ascii=False))
